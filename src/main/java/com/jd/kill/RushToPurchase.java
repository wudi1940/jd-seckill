...
//            setIpProxy();
            ...
            String ip = HttpUrlConnectionUtil.ipList.get(random.nextInt(200) - 1);
            String[] r1 = ip.split(":");
            Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(r1[0], Integer.parseInt(r1[1])));
            ...
            String gate = null;
            try {
                gate = HttpUrlConnectionUtil.get(headers,
                        "https://cart.jd.com/gate.action?pcount=1&ptype=1&pid=" + Start.pid,
                        proxy);
            } catch (IOException e) {
                e.printStackTrace();
            }
            ...
            try {
                String orderInfo = HttpUrlConnectionUtil.get(headers,
            ...
            try {
                if (times < Start.ok) {
                    submitOrder = HttpUrlConnectionUtil.post(headers,
